...
file_path <- "/Users/amirgazar/Documents/GitHub/Decarbonization-Tradeoffs/2 Generation Expansion Model/1 Demand/1 Hourly Demand/representative_days.csv"
Rep_days <- fread(file_path)
...
dispatch_curve <- function(sim, pathway, date, hour, percentile, storage_status, prev_gen) {
  ...
  Nuclear_hour <- Installed_Capacity_hour$Nuclear
  Hydro_hour <- Installed_Capacity_hour$Hydropower
  Bio_hour <- Installed_Capacity_hour$Biomass
  ...
  Fossil_Fuels_gen_hour <- Fossil_Fuels_gen_hour[Retirement_year > year]
  ...
  if (Demand_hour < Clean_gen_hour + SMR_gen_hour) {
    ...
    Demand_for_Fossil_Fuels_hour <- max(0, Demand_hour - Clean_gen_hour - storage_status - SMR_gen_hour)
...
unique_dates <- as.Date(sort(unique(Rep_days$Date)), origin = "1970-01-01")
...
all_simulation_results <- lapply(1:n_simulations, function(sim) {
  ...
  lapply(pathways, function(pathway) {
    ...
    lapply(seq_along(unique_dates), function(date_idx) {
      ...
      # Check if storage_status and prev_gen need to be initialized
      if (!exists("storage_status") || 
          (date_idx != 1 && as.Date(date) != as.Date(unique_dates[date_idx - 1]) + 1)) {
        storage_status <- 0
      }
      if (!exists("prev_gen") || 
          (date_idx != 1 && as.Date(date) != as.Date(unique_dates[date_idx - 1]) + 1)) {
...
write.csv(combined_results_hr, "/Users/amirgazar/Documents/GitHub/Decarbonization-Tradeoffs/2 Generation Expansion Model/5 Dispatch Curve/1 Test Results/Hourly_Results_NE_repdays.csv", row.names = FALSE)
write.csv(combined_results_facility_level, "/Users/amirgazar/Documents/GitHub/Decarbonization-Tradeoffs/2 Generation Expansion Model/5 Dispatch Curve/1 Test Results/Facility_Level_Results_repdays.csv", row.names = FALSE)
