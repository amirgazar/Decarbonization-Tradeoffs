...
simulation_results <- lapply(1:n_simulations, function(sim) {
  ...
  pathway_results <- lapply(pathways, function(pathway) {
    ...
    # Return the results for this pathway
    list(final_hourly = final_hourly_results,
         fossil_fuels = fossil_fuels_hourly_results)
  })
  pathway_results
})

# Combine results across all simulations and pathways
# Flatten the nested list and combine final hourly results
combined_final_hourly_results <- do.call(rbind, lapply(simulation_results, function(sim_results) {
  do.call(rbind, lapply(sim_results, function(res) res$final_hourly)
  )
}))

# Flatten the nested list and combine fossil fuels results
combined_fossil_fuels_results <- do.call(rbind, lapply(simulation_results, function(sim_results) {
  do.call(rbind, lapply(sim_results, function(res) res$fossil_fuels)
  )
}))
...
write.csv(combined_fossil_fuels_results, "/Users/amirgazar/Documents/GitHub/Decarbonization-Tradeoffs/2 Generation Expansion Model/5 Dispatch Curve/1 Test Results/Facility_Level_Results.csv", row.names = FALSE)
