...
# Predefined simulation and pathway
simulation <- 4
pathway <- 1

cat("Running simulation:", simulation, "\n")
cat("Processing pathway:", pathway, "\n")

# Execute the dispatch curve for the current simulation and pathway
dispatch_curve_results <- dispatch_curve(sim = simulation, pathway = pathway)
...
final_hourly_results$Simulation <- simulation
final_hourly_results$Pathway <- pathway
fossil_fuels_hourly_results$Simulation <- simulation
...
hourly_results_path <- sprintf("/projects/epadecarb/2 Generation Expansion Model/4 Results/1 Stepwise/Hourly_Results_sim%d_path%s_time%s.csv", simulation, pathway, timestamp)
facility_level_results_path <- sprintf("/projects/epadecarb/2 Generation Expansion Model/4 Results/1 Stepwise/Facility_Level_Results_sim%d_path%s_time%s.csv", simulation, pathway, timestamp)
