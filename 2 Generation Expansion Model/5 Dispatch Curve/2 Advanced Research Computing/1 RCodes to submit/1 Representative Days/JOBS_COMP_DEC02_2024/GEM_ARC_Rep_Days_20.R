...
file_path <- "/home/amirgazar/Paper2_Nov_2024/Datasets/representative_days.csv"
...
# Representative days processing
...
iteration <- 19
start_sim <- 1 + iteration * 3
n_simulations <- 3 + iteration * 3
...
hourly_results_file <- sprintf("/projects/epadecarb/Decarb_Paper/Results/Rep_Days/Hourly_Results_NE_rep_days_%s_iteration_%s.csv", timestamp, iteration)
facility_level_results_file <- sprintf("/projects/epadecarb/Decarb_Paper/Results/Rep_Days/Facility_Level_Results_rep_days_%s_iteration_%s.csv", timestamp, iteration)
...
response <- POST("https://api.pushover.net/1/messages.json",
                 body = list(
                   ...
                   user = pushover_user,
                   message = "The rep_days_code executed."
                 ),
                 encode = "form")
