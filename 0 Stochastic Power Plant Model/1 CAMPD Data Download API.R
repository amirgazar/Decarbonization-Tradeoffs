...
download_state_emissions_data <- function(apiKEY, years, timeOfLastRun, chunk_size = 1, 
  ...
  for (stateCode in stateCodes) {
    ...
    if (res$status_code > 399) {
      log_message(paste("Error retrieving facilities data for state:", stateCode))
      log_message(paste("HTTP Status Code:", res$status_code))
      next  # Skip to next state
    }
    
    facilitiesData <- fromJSON(rawToChar(res$content), flatten = TRUE)
    if (nrow(facilitiesData) == 0) {
      log_message(paste("No facilities data available for state:", stateCode))
      next
    }
    ...
    log_message(paste("Facilities data for state", stateCode, "saved successfully."))
    ...
    if (res$status_code > 399) {
      log_message(paste("Error retrieving emissions files for state:", stateCode))
      next
    }
    ...
    if (nrow(stateHourlyEmissionsFiles) == 0) {
      log_message(paste("No emissions data available for state:", stateCode))
      next
    }
    
    # Process and save each year separately
    for (yr in years) {
      log_message(paste("Processing emissions data for state:", stateCode, "year:", yr))
      ...
      if (nrow(files_year) == 0) {
        log_message(paste("No emissions data available for state:", stateCode, "year:", yr))
        next
      }
      ...
      for (i in 1:nrow(files_year)) {
        ...
        log_message(paste0("Downloading file for state: ", stateCode, ", year: ", yr, " - ", fileUrl))
        ...
        tryCatch({
          ...
          resp <- GET(fileUrl, write_disk(temp_file, overwrite = TRUE), timeout(3600))
          ...
          log_message(paste("Error reading file:", fileUrl))
          log_message(e)
        }, finally = {
      ...
      output_file <- file.path(yearly_dir, paste0("Hourly_Emissions_", stateCode, "_", yr, ".parquet"))
      write_parquet(stateHourlyEmissionsData_chunk, output_file)
      log_message(paste("Emissions data for state", stateCode, "year", yr, "saved successfully."))
